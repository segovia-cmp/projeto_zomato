...
# Nomeia os paises de acordo com os códigos
def country_name(country_id):
  return COUNTRIES[country_id]

#Classificação dos valores dos pratos
...
def map_location(df):
  ...
  """Esta função cria o mapa com a localização dos restaurantes.

    Utilização do folium para criação do mapa de geolocalização
    df_aux: reune as colunas necessarias para plotagem da localização dos restaurantes
    map: criação do mapa
    markr_cluster: indica a localização para marcação no mapa
  ...
  for index, location_info in df_aux.iterrows():
      folium.Marker([location_info['latitude'],location_info['longitude']],
                  popup=folium.Popup(f"<h5><b>{location_info['restaurant_name']}</b></h5>Média: {location_info['aggregate_rating']}<br>Cuisines: {location_info['cuisines']}", max_width=300,min_width=150),
