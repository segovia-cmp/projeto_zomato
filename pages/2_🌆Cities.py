...
def top_restaurant_city (df):

  """Função para ciração de gráfico com quantidaded e restaurantes por cidade.
    Entrada: dataframe
    Saída: fig (gráfico)
...
with st.container():
        ...
        with col2:
            ...
            fig = rating_mean_down(df)
            st.plotly_chart(fig, use_container_width=True)

            

fig = distinctive_cuisine(df)
st.plotly_chart(fig, use_container_width=True)